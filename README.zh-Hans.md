...
- **同一张卡重复导入不会重复拷贝**——目标位置已存在的相同文件自动跳过
- 同名但不同的照片（计数器归零、两张卡撞名）会并存保留，绝不覆盖
- 可选**「校验通过后把原文件移到废纸篓」**——只清理拷贝并校验都成功的文件
...
- **XMP 附属文件**（可选）：把筛选结果交给 Lightroom / Capture One / Bridge，也可从已有 XMP 反向导入评级
...
### 性能
- 分批流式扫描，发现即显示，不一次性读取全部文件元数据；切卡或拔卡时扫描立即中止
