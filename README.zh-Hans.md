...
> **注意**：在可移除存储卡上，macOS 的废纸篓是**卡上的一个文件夹**，所以移入废纸篓并不会立刻释放卡的空间，需要清空废纸篓才行。想立刻腾出空间，请使用永久删除，或用「导入后删除原文件」。
...
| 无损编辑器 | RAW/JPG 配对删除 |
| --- | --- |
| ![编辑器](docs/screenshots/editor.png) | ![配对删除](docs/screenshots/delete-pairing.png) |
...
- **同时支持视频**（MOV/MP4/M4V/MTS 等），卡上的占用统计与访达一致；大疆/GoPro 这类同名照片+视频也会一并联动删除
...
### 导入到电脑
- 把文件从存储卡拷贝到任意文件夹，**逐个文件做校验和校验**
- 整理方式：`单个文件夹` / `按日期` / `按年月` / `按日期再按类型`（`2026-08-19/RAW`、`/JPEG`、`/Video`）
- **同一张卡重复导入不会重复拷贝**——同名同大小文件还需通过内容校验，确认相同后才跳过
- 同名但不同的照片（计数器归零、两张卡撞名）会并存保留，绝不覆盖
- 可选**「校验通过后把原文件移到废纸篓」**——开启后强制校验，只清理拷贝并校验都成功的文件
- 导入和编辑导出先写入临时文件，完成后发布到目标路径，不覆盖已有文件或原图
- 开始前检查可用空间；中途取消不会留下写了一半的残缺文件
- 拷贝会保留原始拍摄时间
...
### 标记与信息
- 星级（0–5）与彩色标签标记，持久化保存（侧车索引，不修改原文件），按卡隔离命名空间
- **XMP 附属文件**（可选）：单张和批量标记均可同步到 Lightroom / Capture One / Bridge；更新或清空标记会保留已有 XMP 中其他软件的编辑信息，也可反向导入评级
- 编辑器的调整会按照片记住，关掉再打开还在
...
### 性能
- 分批流式扫描，合并更新并增量维护文件索引；筛选、排序在后台执行
- 打开正在预取的照片会提升任务优先级；取消过期邻居预取及排队中的编辑预览
- 标记索引在后台合并保存，正常退出时完成待写入的数据
- 缩略图懒加载 + 基于内存占用的缓存淘汰（默认上限约 256 MB），滚动流畅；按尺寸分档缓存，拖动缩放滑块不会触发重新解码
- 解码请求自动合并、并发受限（瓶颈在读卡器而非 CPU），**快速滚过的请求会被丢弃**——不会因为甩了一下列表就排上几千个已经没人要的解码
- 预加载走独立通道，永远不会挡住你真正翻到的那张
...
- **XMP 附属文件**：评级/打标签时是否同步写入 `.xmp`；从已有附属文件导入标记；为所有已标记文件批量写入
- **界面语言**：跟随系统或手动指定
